...
class User_CollectController extends Yaf_Controller_Abstract {
	...
	public function indexAction() {
		...
		//print_r($leftbars);
		$user_collects = $this->getrecentcollects($this->user_id);
		//print_r($user_collects);
		$user_info = $this->getuserinfo($this->user_id);
		$user_relationships = $this->getuserrelationship($user_info['user_id']);
		//print_r($leftbars);
		
		//echo "</pre>";
		$this->getView()->assign("title"," 我的收藏 - 个人中心 - i比熊  ");
		...
		$this->getView()->assign("post",$user_collects);
		$this->getView()->assign("user_relationships",$user_relationships);
		
		$this->getView()->display(Yaf_Application::app()->getConfig()->smarty->template_dir . "user/collect.tpl");
	...
	public function getrecentcollects($user_id=0) {
		$usermodel = new UserModel();
		$user_collects = array();	
		$collect_ids = array();
		$collect_ids_array = $usermodel->get_postids($user_id,'collect');	
		//print_r($share_ids);
		foreach ($collect_ids_array as $array) {
			$collect_ids[] = $array['user_post_relationship_post_id'];
		}
		
		if ($collect_ids) {
			//$user_collects = $usermodel->get_posts($collect_ids);
			foreach ($collect_ids as $collect_id) {
				$post_info = $usermodel->get_post($collect_id);
				if ($post_info) {
					$user_collects[] = $post_info;
				}
			}
		}
