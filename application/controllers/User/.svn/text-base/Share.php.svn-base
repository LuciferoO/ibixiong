...
class User_ShareController extends Yaf_Controller_Abstract {
	...
	public function indexAction() {
		...
		$this->getView()->assign("post",$user_shares);
	...
	public function getrecentnews($user_id=0) {
		...
		$user_shares = array();	
		$share_ids  = array();
		$share_ids_array = $usermodel->get_postids($user_id,'share');	
		
		foreach ($share_ids_array as $array) {
			$share_ids[] = $array['user_post_relationship_post_id'];
		}
		//print_r($share_ids);
		if ($share_ids) {
			//$user_shares = $usermodel->get_posts($share_ids);
			foreach ($share_ids as $share_id) {
				$post_info = $usermodel->get_post($share_id);
				if ($post_info) {
					$user_shares[] = $post_info;
				}
			}
		}
